...
When installing Camel JBang using `jbang app install`, JBang will occasionally check
upstream sources and update the installed application automatically. This may be undesirable if you
want to ensure your Camel JBang installation remains on a specific Camel release.
...
TIP: For a complete reference of all commands and their options, see the xref:jbang-commands/camel-jbang-commands.adoc[Camel JBang Command Reference].
...
However, Camel JBang may be used as a tool for migrating existing Maven based projects.
...
The `camel cmd load` command can be used to load new route(s) into an existing running Camel JBang application.
For example during development to inject a new route for experimentation. This can also be used as part
of testing scenarios using Citrus with the xref:camel-jbang-test.adoc[Camel JBang Test] plugin.
...
NOTE: You MUST include the properties files in the files path when using `camel run` or `camel export` to include the files, such as `camel run hello.java application.properties`.
...
And yet again it was changed in *Camel 4.12* to be a single number by default.
...
Camel JBang provides a plugin for writing and executing automated tests.
...
Once the prototyping phase with Camel JBang is complete you can include the automated tests into the project export.
...
This tells Camel JBang to look inside the Maven pom.xml file and if `camel-spring-boot-bom` is detected,
then Camel JBang will automatically run Spring Boot with debugging enabled. This is done by letting Spring Boot
...
| *camel.jbang.fresh* | Whether to force usin fresh (i.e. non-cached) resources | true | boolean
...
| *camel.jbang.quarkusArtifactId* | artifactId of Camel Quarkus Platform BOM |  | String
| *camel.jbang.quarkusExtensioRegistryBaseUri* | The base URI of Quarkus Extension Registry |  | String
| *camel.jbang.quarkusGroupId* | groupId of Quarkus Platform BOM |  | String
| *camel.jbang.quarkusGroupId* | groupId of Camel Quarkus Platform BOM |  | String
| *camel.jbang.quarkusVersion* | version of Quarkus Platform BOM |  | String
| *camel.jbang.quarkusVersion* | version of Camel Quarkus Platform BOM |  | String
...
In Camel 4.14 then support for using Groovy source together with Camel JBang has been improved.
